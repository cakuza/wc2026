  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[rgba(14,12,10,.10)] bg-white/92 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 py-3">
        ...
        <nav className="flex flex-1 gap-1 overflow-x-auto px-1 text-sm text-[#0E0C0A]/70">
          {navItems.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-md px-3 py-2 transition hover:bg-[#0E0C0A]/5 hover:text-[#0E0C0A]"
            >
              {item.label}
            </Link>
          ))}

...
async function run(): Promise<void> {
  ...
  function buildStats(liveData: Map<number, LiveMatchData>) {
    const data = liveData.get(TEST_MATCH_ID)!;
    const goalEventCompleteness = getGoalEventCompleteness({
      homeScore: data.homeScore,
      awayScore: data.awayScore,
      goals: data.goals,
      eventDataAvailable: data.eventDataAvailable,
    });
    const matches = {
      "mexico-vs-south-africa-jun11": {
        status: data.status === "FINISHED" ? "FINISHED" : "SCHEDULED",
        goalEventCompleteness,
        scorers: (data.goals ?? []).map((goal) => ({
          playerName: goal.playerName ?? "Scorer unavailable",
          confidence: data.eventDataAvailable ? "high" : "low",
          isOwnGoal: goal.isOwnGoal,
          playerTeamName: goal.playerTeamName,
        })),
      },
    } as unknown as Record<string, SerializableSnapshotMatch>;
    return computeTournamentStats(liveData, matches);
  }
  ...
  await test("matchesPlayed=1 for one FINISHED match", async () => {
    const stats = buildStats(makeLiveData({ status: "FINISHED", homeScore: 1, awayScore: 0 }));
    assert.strictEqual(stats.matchesPlayed, 1);
  });

  await test("matchesPlayed=0 for one IN_PLAY (live) match", async () => {
    const stats = buildStats(makeLiveData({
      status: "IN_PLAY",
  ...
  await test("matchesPlayed=0 for one SCHEDULED match", async () => {
    const stats = buildStats(makeLiveData({ status: "SCHEDULED", homeScore: null as any, awayScore: null as any, goals: [] }));
    assert.strictEqual(stats.matchesPlayed, 0,
  ...
  await test("totalGoals matches canonical score sum (2+1=3)", async () => {
    ...
    assert.strictEqual(stats.totalGoals, 3,
  ...
  await test("totalGoals=0 when no FINISHED matches", async () => {
    const stats = buildStats(makeLiveData({ status: "IN_PLAY", homeScore: 2, awayScore: 0 }));
    assert.strictEqual(stats.totalGoals, 0);
  });
  ...
  await test("scorerTotalsComplete=true when all scorer events match canonical score", async () => {
    ...
    assert.ok(stats.scorerTotalsComplete,
  ...
  await test("scorerTotalsComplete=false when scorer events are fewer than canonical score", async () => {
    ...
    assert.ok(stats.completedMatchesWithUnresolvedScorers >= 1,
      "completedMatchesWithUnresolvedScorers must be ≥1");
    assert.ok(stats.unresolvedCompletedMatchGoals >= 1,

...
async function runTests() {
  ...
  const tMatches = fs.readFileSync('components/MatchCenterContent.tsx', 'utf8');
  const mDetail = fs.readFileSync('components/MatchDetail.tsx', 'utf8');
  const ticker = fs.readFileSync('components/Ticker.tsx', 'utf8');

  assert.ok(!tMatches.includes('router.refresh'), 'no router.refresh in MatchCenterContent');
